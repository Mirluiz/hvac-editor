...
var _2Pipes = function (model, pipes, step) {
  ...
  __spreadArray(__spreadArray([], arraysRL90, true), arraysLR90, true).map(
    function (lines, index) {
      lines.map(function (line) {
        pipes.push(
          new pipe_model_1.default(
            model,
            new vect_1.Vector(100 * index + line.x1 * step, line.y1 * step),
            new vect_1.Vector(100 * index + line.x2 * step, line.y2 * step)
          )
        );
      });
    }
  );
  __spreadArray(__spreadArray([], arraysV90Down, true), arraysV90Up, true).map(
    function (lines, index) {
      lines.map(function (line) {
        pipes.push(
          new pipe_model_1.default(
            model,
            new vect_1.Vector(
              100 * index + line.x1 * step,
              12 * step + line.y1 * step
            ),
            new vect_1.Vector(
              100 * index + line.x2 * step,
              12 * step + line.y2 * step
            )
          )
        );
      });
    ...
    lines.map(function (line) {
      pipes.push(
        new pipe_model_1.default(
          ...
          new vect_1.Vector(
            100 * index + line.x1 * step,
            18 * step + line.y1 * step
          ),
          new vect_1.Vector(
            100 * index + line.x2 * step,
            18 * step + line.y2 * step
          )
        )
      );
    });
    ...
    lines.map(function (line) {
      pipes.push(
        new pipe_model_1.default(
          ...
          new vect_1.Vector(
            100 * index + line.x1 * step,
            22 * step + line.y1 * step
          ),
          new vect_1.Vector(
            100 * index + line.x2 * step,
            22 * step + line.y2 * step
          )
        )
      );
    });
  ...
  [0, 30, 60, 90].map(function (a, index) {
    ...
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(
          400 + 100 * index + pV1.x * step,
          22 * step + pV1.y * step
        ),
        new vect_1.Vector(
          400 + 100 * index + pV2.x * step,
          22 * step + pV2.y * step
        )
      )
    );
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(
          400 + 100 * index + v1.x * step,
          22 * step + v1.y * step
        ),
        new vect_1.Vector(
          400 + 100 * index + v2.x * step,
          22 * step + v2.y * step
        )
      )
    );
  ...
  [0, 30, 60, 90, 120, 150, 180].map(function (a, index) {
    ...
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(100 * index + pV1.x * step, 32 * step + pV1.y * step),
        new vect_1.Vector(100 * index + pV2.x * step, 32 * step + pV2.y * step)
      )
    );
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(100 * index + v1.x * step, 32 * step + v1.y * step),
        new vect_1.Vector(100 * index + v2.x * step, 32 * step + v2.y * step)
      )
    );
  });
  [0, -30, -60, -90, -120, -150, -180].map(function (a, index) {
    ...
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(100 * index + pV1.x * step, 42 * step + pV1.y * step),
        new vect_1.Vector(100 * index + pV2.x * step, 42 * step + pV2.y * step)
      )
    );
    pipes.push(
      new pipe_model_1.default(
        model,
        new vect_1.Vector(100 * index + v1.x * step, 42 * step + v1.y * step),
        new vect_1.Vector(100 * index + v2.x * step, 42 * step + v2.y * step)
      )
    );
  });
};
